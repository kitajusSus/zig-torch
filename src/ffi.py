...
try:
    ...
    print(
        "Sprawdź, czy funkcja jest poprawnie zadeklarowana jako 'pub export' w kodzie Zig."
    )
    exit(1)
...
def multiply_matrices_with_zig(A_np: np.ndarray, B_np: np.ndarray) -> np.ndarray:
    ...
    # macierz wynikową C (zaalokowaną przez Python/Numpy)
...
if __name__ == "__main__":
    ...
    print(
        f"Creating Matrix A ({M_dim}x{K_dim}) and B ({K_dim}x{N_dim}) type float32..."
    )
